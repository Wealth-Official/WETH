...
contract LandlordFall is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isLandlord;
    mapping (address => bool) private _isExcludedFromLandlord;
    address[] private _landlord;
    uint256 public landlordLine = 2500000 * 10**6 * 10**9;
    uint256 public constant landlordPriceBase = 5714285714;
    uint256 public priceDeductCount;
    uint256 public _landlordDeductRate = 1;
    ...
    event AddLandlord(address landlord);
    event BurnLandlord(address landlord, uint256 amount);
    ...
    constructor (address _uniswapRouter, address _usdtAddress, address _bnbAddress) public {
        ...
        _isExcludedFromLandlord[owner()] = true;
        _isExcludedFromLandlord[address(this)] = true;
    ...
    function getLandlordPerUSDTPrice(uint amountIn) public view returns (uint amountOut) {
    ...
    function getUSDTPerLandlordPrice(uint amountIn) public view returns (uint amountOut) {
        IUniswapV2Pair pair = IUniswapV2Pair(uniswapV2Pair);
        address token0 = pair.token0();
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = pair.getReserves();
    ...
    function isLandlord(address account) public view returns (bool) {
        return _isLandlord[account];
    }
    ...
    function _burnLandlord(address landlordAddress, uint256 tAmount) private {
        require(_isLandlord[landlordAddress], "address is not landlord");
        (uint256 rAmount,,,,,) = _getValues(tAmount);
        _rOwned[landlordAddress] = _rOwned[landlordAddress].sub(rAmount);
        _tOwned[landlordAddress] = _tOwned[landlordAddress].sub(tAmount);
        _rOwned[address(0)] = _rOwned[address(0)].add(rAmount);
        emit BurnLandlord(landlordAddress, tAmount);
    }

    function _burnAllLandlord() private {
        for (uint256 i = 0; i < _landlord.length; i++) {
            _burnLandlord(_landlord[i], calculateLandlordDeductFee(_tOwned[_landlord[i]]));
        }
    }

    function triggerBurnLandlord() public onlyOwner{
        ...
        uint p = getUSDTPerLandlordPrice(1*10**9).div(landlordPriceBase);
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        require(!_isLandlord[account], "Landlord Account can't excluded");
    ...
    function addLandlord(address account) public onlyOwner {
        require(!_isLandlord[account], "Account is landlord");
        require(!_isExcluded[account], "Excluded account can't be landlord");
        require(!_isExcludedFromLandlord[account], "Account is excluded from landlord");
        require(!isExchangeAddr(account), "ExchangeAddr can not be landlord");
        _addLandlord(account);
    }

    function _addLandlord(address account) private {
        _isLandlord[account] = true;
        _landlord.push(account);
        _tOwned[account] = tokenFromReflection(_rOwned[account]);
        emit AddLandlord(account);
    }

    function removeLandlord(address account) external onlyOwner {
        require(_isLandlord[account], "Account is not landlord");
        for (uint256 i = 0; i < _landlord.length; i++) {
            if (_landlord[i] == account) {
                _landlord[i] = _landlord[_landlord.length - 1];
                _isLandlord[account] = false;
                _landlord.pop();
                _tOwned[account] = 0;
                break;
            }
    ...
    function excludeFromLandlord(address account) public onlyOwner {
        _isExcludedFromLandlord[account] = true;
    }

    function includeInLandlord(address account) public onlyOwner {
        _isExcludedFromLandlord[account] = false;
    }
    ...
    function setLandlordDeductRate(uint256 landlordDeductRate) external onlyOwner() {
        _landlordDeductRate = landlordDeductRate;
    }
    ...
    function setLandlordLine(uint256 _landlordline) external onlyOwner() {
        landlordLine = _landlordline;
    }
    ...
    function calculateLandlordDeductFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_landlordDeductRate).div(
            10**2
        );
    }
    ...
    function isExcludedFromLandlord(address account) public view returns(bool) {
        return _isExcludedFromLandlord[account];
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner()){
            ...
            {
                require(amount <= getLandlordPerUSDTPrice(transferUSDTLimit), "Transfer amount exceeds the initMaxTxAmount.");
            }
        ...
        if(
            !isExchangeAddr(to) &&
        balanceOf(to)>=landlordLine &&
        !isLandlord(to) &&
        !_isExcludedFromLandlord[to]
        )
        {
            revert('Can not hold more than landlordLine');
        }
    }
